...
namespace BlogAppFrontend.ApiServices.Concrete
{
    public class BlogApiManager : IBlogApiService
    {
        ...
        public BlogApiManager(HttpClient httpClient, IHttpContextAccessor httpContextAccessor)
        {
            ...
            _httpClient.BaseAddress = new Uri("http://localhost:60008/api/blogs/");
        ...
        public async Task AddAsync(BlogAddModel model)
        {
            ...
            if (model.Image != null)
            {
                // var imgP = $"C:\\Users\\ramazan\\Desktop\\{model.Image.FileName}";
                // var bytes = await System.IO.File.ReadAllBytesAsync(model.Image.FileName);
                // var bytes = await System.IO.File.ReadAllBytesAsync(tempByte);
                byte[] fileBytes;
        ...
        public async Task<List<CommentListModel>> GetCommentAsync(int blogId, int? parentCommentId)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<List<CommentListModel>>(await responseMessage.Content
                    .ReadAsStringAsync());
            }
